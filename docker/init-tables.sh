...
create_table_if_not_exists() {
  ...
  if aws dynamodb list-tables --endpoint-url $ENDPOINT --region $REGION | grep -q "\"$TABLE_NAME\""; then
    echo "Table $TABLE_NAME already exists. Skipping creation."
  else
    echo "Creating $TABLE_NAME..."
    aws dynamodb create-table --cli-input-json file://$JSON_FILE --endpoint-url $ENDPOINT --region $REGION
  fi
}
...
set_ttl_if_not_set() {
  ...
  TTL_STATUS=$(aws dynamodb describe-time-to-live --table-name $TABLE_NAME --endpoint-url $ENDPOINT --region $REGION | jq -r '.TimeToLiveDescription.TimeToLiveStatus')
