import { DynamoDBClient } from '../../../functions/counter-function/node_modules/@aws-sdk/client-dynamodb';
import {
  DynamoDBDocumentClient, PutCommand
} from "../../../functions/counter-function/node_modules/@aws-sdk/lib-dynamodb";
...
describe('Counter Function MessageRepository Integration Tests', () => {
  ...
  describe('getMessagesFromTimeStampRange', () => {
    test('指定した時間範囲のメッセージを取得できること', async () => {
      ...
      const testMessages = [
        { MessageNo: 1, UserId: 'user1', Content: 'Message 1', CreatedAt: baseTime - 3600, Dummy: 'ALL' }, // 1時間前
        { MessageNo: 2, UserId: 'user2', Content: 'Message 2', CreatedAt: baseTime - 1800, Dummy: 'ALL' }, // 30分前  
        { MessageNo: 3, UserId: 'user1', Content: 'Message 3', CreatedAt: baseTime - 600, Dummy: 'ALL' },  // 10分前
        { MessageNo: 4, UserId: 'user3', Content: 'Message 4', CreatedAt: baseTime + 600, Dummy: 'ALL' },  // 10分後（未来）
      ];
      ...
      const result = await messageRepository.getMessagesFromTimeStampRange(startTime, endTime);
    ...
    test('範囲外の場合は空配列を返すこと', async () => {
      ...
      await docClient.send(new PutCommand({
        TableName: process.env.MESSAGE_TABLE || 'MessageTable',
        Item: { MessageNo: 1, UserId: 'user1', Content: 'Message 1', CreatedAt: baseTime, Dummy: 'ALL' }
      }));

      // Act: 範囲外の時刻で検索
      const result = await messageRepository.getMessagesFromTimeStampRange(baseTime + 3600, baseTime + 7200);
    ...
    test('同じ時刻の複数メッセージが正しく取得されること', async () => {
      ...
      const messages = [
        { MessageNo: 1, UserId: 'user1', Content: 'Message 1', CreatedAt: sameTime, Dummy: 'ALL' },
        { MessageNo: 2, UserId: 'user2', Content: 'Message 2', CreatedAt: sameTime, Dummy: 'ALL' },
        { MessageNo: 3, UserId: 'user3', Content: 'Message 3', CreatedAt: sameTime, Dummy: 'ALL' },
      ];
      ...
      // Act: 同じ時刻を含む範囲で検索
      const result = await messageRepository.getMessagesFromTimeStampRange(sameTime, sameTime);
  ...
  describe('パフォーマンステスト', () => {
    test('大量データでも正常に動作すること', async () => {
      ...
      // Act: 広い範囲で検索
      const result = await messageRepository.getMessagesFromTimeStampRange(baseTime - 3000, baseTime);
