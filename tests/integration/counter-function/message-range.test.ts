...
import { MessageRangeRepository, MessageRangeItems } from "../../../functions/counter-function/repository/message-range";
...
describe('Counter Function MessageRangeRepository Integration Tests', () => {
  ...
  beforeEach(async () => {
    // 各テスト前にテーブルをクリーンアップ
    await cleanUP('CounterRangeTable');
  });
  ...
  describe('saveCounter', () => {
    test('集計データを正しく保存できること', async () => {
      ...
      // Act
      await expect(messageRangeRepository.saveCounter(testData)).resolves.not.toThrow();
      ...
      expect(result.Item!.ExpirationDate).toBe(Math.floor(testData.ExpirationDate.getTime() / 1000));
    });
    ...
    test('ExpirationDateがUnixタイムスタンプに正しく変換されること', async () => {
      ...
      expect(result.Item).toBeDefined();
      expect(result.Item!.ExpirationDate).toBe(Math.floor(futureDate.getTime() / 1000));
      
      // 元のDateオブジェクトと同じ時刻を表していることを確認
      const savedDate = new Date(result.Item!.ExpirationDate * 1000);
      expect(Math.floor(savedDate.getTime() / 1000)).toBe(Math.floor(futureDate.getTime() / 1000));
    });
