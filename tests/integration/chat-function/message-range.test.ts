...
import {
  ...
  MessageRangeItems,
} from "../../../functions/chat-function/repository/message-range";
import {
...
describe("MessageRangeRepository Integration Tests", () => {
  ...
  describe("getAllCounters", () => {
      ...
      const testData = [
        {
          ...
          ExpirationDate: Math.floor(
            new Date("2024-12-21T00:00:00Z").getTime() / 1000
          ), // Unix timestamp
        },
        {
          ...
          CreatedAt: Math.floor(Date.now() / 1000),
          ExpirationDate: Math.floor(
            new Date("2024-12-21T06:00:00Z").getTime() / 1000
          ),
        },
      ];
    ...
    test("ExpirationDateがUnix timestampからDateオブジェクトに正しく変換されること", async () => {
      ...
      const expirationTimestamp = Math.floor(
        new Date("2024-12-25T10:30:00Z").getTime() / 1000
      );
      const testData = {
      ...
      expect(result[0].ExpirationDate.getTime()).toBe(
      ...
      for (let day = 20; day <= 22; day++) {
        for (let hour = 0; hour < 24; hour += 6) {
          const startTime = new Date(
            `2024-11-${day}T${hour.toString().padStart(2, "0")}:00:00Z`
          );
          const endTime = new Date(startTime.getTime() + 6 * 60 * 60 * 1000); // +6時間

          testRanges.push({
            RecordId: `range-2024-11-${day}-${hour
              .toString()
              .padStart(2, "0")}`,
  ...
  describe("データ構造テスト", () => {
    test("MessageRangeItemsの型定義に従ったデータが取得されること", async () => {
      ...
      expect(item.ExpirationDate).toEqual(
        new Date(testData.ExpirationDate * 1000)
      );
    });
  });
  ...
  describe("異常系テスト", () => {
    test("無効なExpirationDateでもエラーにならないこと", async () => {
      ...
      await expect(
        messageRangeRepository.getAllCounters()
