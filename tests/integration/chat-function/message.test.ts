...
import { MessageRepository, MessageItem } from '../../../functions/chat-function/repository/message';
...
describe('MessageRepository Integration Tests', () => {
  ...
  const messageTableName = 'MessageTable';
  const counterTableName = 'MessageCounterTable';
  ...
  beforeEach(async () => {
    // 関連する両テーブルをクリーンアップ
    await cleanUP(messageTableName);
    await cleanUP(counterTableName);
  });
  ...
  describe('putMessage', () => {
    test('メッセージを正しく登録できること', async () => {
      ...
      const messages = await messageRepository.getMessagesByPage(1, 10, messageNo);
    ...
    test('複数のメッセージを連続で登録できること', async () => {
      ...
      // Assert
      const messages = await messageRepository.getMessagesByPage(1, 10, Math.max(...messageNos));
  ...
  describe('getMessagesByPage', () => {
    ...
    test('異常なパラメータでエラーを投げること', async () => {
      await expect(messageRepository.getMessagesByPage(0, 5, 10)).rejects.toThrow('page must be >= 1');
      await expect(messageRepository.getMessagesByPage(1, 0, 10)).rejects.toThrow('perPage must be >= 1');
      await expect(messageRepository.getMessagesByPage(1, 5, -1)).rejects.toThrow('max must be >= 0');
    });
  });
  ...
  describe('getMessagesFromLast', () => {
    ...
    test('異常なパラメータでエラーを投げること', async () => {
      // last < 0
      await expect(messageRepository.getMessagesFromLast(-1, 5)).rejects.toThrow('last must be >= 0');
      
      // max < 0 (修正後はパラメータ検証が先に実行される)
      await expect(messageRepository.getMessagesFromLast(0, -1)).rejects.toThrow('max must be >= 0');
      
      // limit < 1
      await expect(messageRepository.getMessagesFromLast(0, 5, 0)).rejects.toThrow('limit must be >= 1');
    });
  });
  ...
  describe('getMessagesFromTimeStampRange', () => {
    test('指定した時間範囲のメッセージを取得できること', async () => {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        const { PutCommand } = await import('../../../functions/chat-function/node_modules/@aws-sdk/lib-dynamodb');
  ...
  describe('統合テスト', () => {
    test('メッセージ登録からページング取得まで一連の流れが正常に動作すること', async () => {
      ...
      const page1 = await messageRepository.getMessagesByPage(1, 5, currentCounter);
      expect(page1).toHaveLength(5);
      expect(page1.map(m => m.MessageNo)).toEqual([7, 6, 5, 4, 3]);

      const page2 = await messageRepository.getMessagesByPage(2, 5, currentCounter);
      expect(page2).toHaveLength(2);
      expect(page2.map(m => m.MessageNo)).toEqual([2, 1]);

      // lastから取得確認
      const fromLast = await messageRepository.getMessagesFromLast(5, currentCounter);
