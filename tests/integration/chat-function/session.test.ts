...
import {
  SessionRepository,
  SessionItem,
} from "../../../functions/chat-function/repository/session";
import {
...
describe("SessionRepository Integration Tests", () => {
  ...
  describe("upsertSession", () => {
    test("新しいセッションを正しく作成できること", async () => {
      ...
      const retrievedSession = await sessionRepository.getSession(
        sessionData.SessionId
      );

      expect(retrievedSession).toBeDefined();
      expect(retrievedSession!.SessionId).toBe(sessionData.SessionId);
      expect(retrievedSession!.ExpirationDate).toEqual(
    ...
    test("既存のセッションを更新できること", async () => {
      ...
      expect(retrievedSession!.ExpirationDate).toEqual(
        updatedSession.ExpirationDate
      );
      // 初期の日時ではないことを確認
      expect(retrievedSession!.ExpirationDate).not.toEqual(
        initialSession.ExpirationDate
      );
    });
  ...
  describe("getSession", () => {
    ...
    test("日時変換が正しく動作することを確認", async () => {
      ...
      const expectedTimestamp =
        Math.floor(specificDate.getTime() / 1000) * 1000;
  ...
  describe("統合テスト", () => {
    test("セッション作成→取得→更新→取得の一連の流れが正常に動作すること", async () => {
      ...
      expect(retrieved2!.ExpirationDate).not.toEqual(
        initialSession.ExpirationDate
      );
    });
    ...
    test("複数セッションの並行操作が正常に動作すること", async () => {
      ...
      const sessions = Array.from({ length: 5 }, (_, i) => ({
        ...
        ExpirationDate: new Date(
          `2024-12-${(i + 1).toString().padStart(2, "0")}T12:00:00Z`
        ),
      }));
      ...
      const retrievedSessions = await Promise.all(
        sessions.map((session) =>
          sessionRepository.getSession(session.SessionId)
        )
      );
      ...
      retrievedSessions.forEach((retrieved, index) => {
        ...
        expect(retrieved!.ExpirationDate).toEqual(
          sessions[index].ExpirationDate
        );
      });
    });
  });
  ...
  describe("異常系テスト", () => {
    ...
    test("空文字列のSessionIdでは適切にエラーがスローされること", async () => {
      ...
      await expect(
        sessionRepository.upsertSession(emptySessionData)
      ).rejects.toThrow("SessionId cannot be empty");
      await expect(sessionRepository.getSession("")).rejects.toThrow(
        "SessionId cannot be empty"
      );

      await expect(
        sessionRepository.upsertSession(whitespaceSessionData)
      ).rejects.toThrow("SessionId cannot be empty");
      await expect(sessionRepository.getSession("   ")).rejects.toThrow(
        "SessionId cannot be empty"
      );
    ...
    test("過去の日付のExpirationDateでも正常に処理されること", async () => {
      ...
      // Act & Assert
      await expect(
        sessionRepository.upsertSession(pastSession)
      ).resolves.not.toThrow();

      const retrieved = await sessionRepository.getSession(
        pastSession.SessionId
      );
      expect(retrieved).toBeDefined();
    ...
    test("極端に遠い未来の日付でも正常に処理されること", async () => {
      ...
      // Act & Assert
      await expect(
        sessionRepository.upsertSession(futureSession)
      ).resolves.not.toThrow();
